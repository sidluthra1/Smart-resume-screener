...
def openAIParser(src: Path):
    ...
    response = client.responses.create(
        ...
        input="Please parse the job for all the information available from the attached PDF and return EXACTLY the JSON via the function. The summary property should include 1 summarizing sentence about the job. You are also to interperit the Job Category as well.  For any properties that are not available in the resume, please just respond with N/A.",
        tools=[
            {
                "type": "file_search",
